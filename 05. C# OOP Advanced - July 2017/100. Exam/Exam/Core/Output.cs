...
public class Output
{
    public void GiveOutput(StringBuilder result, Dictionary<string, List<ISoldier>> army, Dictionary<string, List<IAmmunition>> wearHouse, int missionQueueCount)
    {
        var orderedArmy = army.OrderBy(s => s.Value.Average(a => a.OverallSkill));
        ...
        foreach (var team in orderedArmy)
        {
            ...
            {
                result.Append($"{soldier.Name} {soldier.OverallSkill}, ");
            }
